...
var AmidaSchema = new Schema({
  ...
  , items: [ItemsSchema]
  // 利用者パスワード
  , userPass: {type: String, required: false}
  // URL
 ...
 * @param int i 文字数
 */
Manager.prototype.makeRandomUrlParam = function(i, callback) {
    
    var st = 'ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz23456789';  // 文字リスト    
    var urlParam = '';
    for (; i > 0; i--) {
      for (; i > 0; i--) {
        // 文字リストからランダムに選択
        var j = Math.floor(Math.random() * st.length);
    
        // 結果文字列に追加
        urlParam += st.charAt(j);
      }
      // 作成された文字列が既にDBに登録されてないか確認する。
      this.find(urlParam, function(err, amidas) {
      
        var amida = amidas[0];
        if (amida) {
          console.log(amida);
        } else {
          callback(null, urlParam);
        }
      });
    }
}
